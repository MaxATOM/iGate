...
- (void)updateIP {
    ...
    NSLog(@"cc");
    if([[NSUserDefaults standardUserDefaults] boolForKey:@"iGate.auto"] || [[NSUserDefaults standardUserDefaults] objectForKey:@"iGate.auto"] == nil) {
        host = @"raspberrypi.local";
        NSLog(@"%@", host);
        port = 8000;
    } else {
        NSLog(@"cc");
        host = [[NSUserDefaults standardUserDefaults] objectForKey:@"iGate.host"];
        port = [[[NSUserDefaults standardUserDefaults] objectForKey:@"iGate.port"] intValue];
        
    }
    NSLog(@"%@ et %d", host, port);
